...
@dataclass
class MusicConfig:
    service_ip: str = os.getenv("MUSIC_SERVICE_IP", "10.226.90.120")
    service_node_id: str = os.getenv("MUSIC_SERVICE_NODE_ID", "NMIC_MUSIC_CMADAAS")
    user_id: str = os.getenv("MUSIC_USER_ID", "")
    password: str = os.getenv("MUSIC_PASSWORD", "")
    timeout: int = int(os.getenv("MUSIC_TIMEOUT", "120"))
...
class MusicClient:
    def __init__(self, config: MusicConfig):
        if not config.user_id or not config.password:
            raise ValueError("请先设置环境变量 MUSIC_USER_ID 和 MUSIC_PASSWORD")
        self.config = config
...
def evaluate_observation_response(
    ...
    def judge(window_hours: int, threshold_mm: float, ratio_threshold: float, level: str, rain_label: str) -> Optional[Dict[str, Any]]:
        ...
        adjacent_ids = find_adjacent_qualified_station_ids(records, qualified_ids, neighbor_km=neighbor_km)
