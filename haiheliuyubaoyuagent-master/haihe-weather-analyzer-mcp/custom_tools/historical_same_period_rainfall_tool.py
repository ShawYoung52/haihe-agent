...
def _no_data_response(ref_start: datetime, ref_end: datetime, years: int, message: str = "历史同期暂无有效自动站降雨量数据。", debug_reason: str = "") -> dict:
    return {
        "status": "no_data",
...
def _station_name(record: dict) -> str:
    return _safe_text(record.get("Station_Name")) or _safe_text(record.get("station_name")) or _safe_text(record.get("Station_Id_C")) or "未知站点"
