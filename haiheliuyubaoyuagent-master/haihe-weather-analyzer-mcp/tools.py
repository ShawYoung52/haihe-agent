...
def register_tools(mcp: FastMCP):
    ...
    def get_city_rainfall_time_range(
        ...
        ⚠️ 注意：数据源按可用性自动切换（汛期滚动预报网格 / 平时 EC）。工具返回的 `data_source`
        字段标注实际数据源，回答时 **必须引用 data_source 的值**，禁止硬编码为 "ECMWF AIFS"。
    ...
    def get_river_system_rainfall_forecast(
        ...
        无论今天、明天还是未来，凡是"海河流域""流域"或具体河系/子流域对象的天气/降雨问题，
        一律使用本工具；禁止使用 query_rolling_forecast（那是天津 11 站点接口，不覆盖海河流域）。
