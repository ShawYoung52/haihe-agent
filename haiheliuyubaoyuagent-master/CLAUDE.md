...
- 测试：`haihe-weather-analyzer-mcp/tests/test_composite_longimg_tool.py`（23 条，含黑底转白底/白底浅灰字/白底黑字保留/绿区白字/白底孤立色块无暗晕 5 类 `_to_white_map` 用例 + 板块容错/预报回退/参数透传）。测试经 importlib 按文件路径加载模块绕开 `custom_tools/__init__.py` 重依赖；本机 `.venv-test` 缺 networkx/rasterio 等重依赖，**只有该文件的测试能全量跑**（其余 `tests/test_*.py` 在 collection 阶段 import `custom_tools/__init__.py` 报 ModuleNotFoundError，属环境限制非回归）。
