...
- **`_to_white_map` 判定原理（勿回退形态学开运算）**：白字与白背景像素值相同，只能靠**局部对比**区分——图例刻度白字紧贴色标条、绿区白字在雨区内部，都离饱和像素近；页面底/图边距等大块白背景远离一切饱和像素。故对饱和内容（chroma≥40）做 `MaxFilter(dilate)` 膨胀成"近邻带"，白底图亮字（≥245）只有落入近邻带才转深，大块白背景保留；灰字（90..245）恒转深；大型近黑区块（零值填充区）经 `MinFilter+MaxFilter` 开运算后填最浅绿（`_lightest_green`）；黑底图（dark_fraction≥0.35）走黑→白底、浅字→深字分支。**勿回退到"用开运算保大块白背景"**——开运算会把紧贴色标条的白字"复活"回白背景而漏改（此前 5 次返工的根因）。
- **前端展示**：`_run_tool_round` 特判 `generate_haihe_composite_longimg`（:2260），`_render_base64_tool_image` 出图；`no_data`/`error` 中文兜底。
- 测试：`haihe-weather-analyzer-mcp/tests/test_composite_longimg_tool.py`（23 条，含黑底转白底/白底浅灰字/白底黑字保留/绿区白字/图例刻度白字 5 类 `_to_white_map` 用例 + 板块容错/预报回退/参数透传）。测试经 importlib 按文件路径加载模块绕开 `custom_tools/__init__.py` 重依赖；本机 `.venv-test` 缺 networkx/rasterio 等重依赖，**只有该文件的测试能全量跑**（其余 `tests/test_*.py` 在 collection 阶段 import `custom_tools/__init__.py` 报 ModuleNotFoundError，属环境限制非回归）。
