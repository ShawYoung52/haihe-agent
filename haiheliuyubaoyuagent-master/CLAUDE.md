...
- **POI 注意事项（决策天气）:** `query_decision_weather_for_poi` 与 `DecisionWeatherQAService` 在 `_generate_decision_weather_answer` 尾部、表格与数据来源之间追加代码确定性生成的 `⚠ 注意事项`（`_build_poi_reminder_section`）。类别由 `classify_poi_category(name, address, category_1, category_2)` 判定（school→airport→station→port→reservoir→scenic→mountain 优先级，mountain 含蓟州/蓟县区域级山洪地灾区、port 含裸"港"（天津港）、reservoir 认"水库"，mountain/station 只认复合词防"石家庄/唐山/裸站"误判）；scenic 另含 `_POI_KNOWN_SCENIC_NAMES` 知名天津景点名单（五大道/古文化街/意式风情区/天津之眼等，**只对 name 匹配**，名称含 街道/办事处/政务/社区/派出所/医院/村/县/镇/乡 时跳过；裸"盘山"歧义已移除）。**POI 定位区域偏好**：`_decision_pick_first_poi(payload, keyword)` 同名大众点（河西中心/实验中学等）默认主场天津优先；keyword 显式含"天津"时**严格过滤**（只认 名称含天津/地址含天津市/区县名且无外省证据词 `_POI_NON_TIANJIN_REGION_RE` 的点位，宁可查不到也不拿外省同名点冒充）；显式含外省城市/省份词（`_POI_EXPLICIT_REGION_WORDS`，城市在前省份在后）则尊重该城市。`category is not None` 时才调 MCP 工具 `query_poi_hazard_reminders`，工具缺失/失败静默跳过。**隐患点只在预报有降雨时提醒**（`_decision_rain_intensity` 分档，`show_hazard = has_hazard and intensity>0`），且只出**风险研判表**（类型×数量×风险×建议，`_HAZARD_RAIN_RISK` dzzh/sh/zxhl 纯规则矩阵零编造），**不逐条列举隐患点**；累计 0mm 不打印"0 毫米"标题。天气断言只来自 facts 数值，隐患点信息只来自工具返回，**禁止 LLM 编造**。两入口接线保持 parity。 “reservoir”类别额外查 14所 水库水位（query_water_level data_type=reservoir，双入口 + orchestrator 历史直调都接线），_build_poi_reminder_section 在注意事项追加“目前X水库库上水位约 Y 米（汛限水位 Z 米）/蓄水量/出库流量”，数值来自接口、无数据不编造。
...
### 14所降水专题组合长图（2026-08-17~18，新功能接入）

接入 14所 7 个降水专题接口，让用户说「长图/组合长图/降水专题长图/出今天的长图/出一张长图」时把多个板块拼成**一张组合长图**（Pillow 模板化版式，对齐示范图 `download.png`）。板块从上到下：降水实况文字 / swan3 组合反射率雷达图 / 降水实况图(③) / 实况面雨量图(④) / 点雨量列表(⑤) / 预报面雨量图(⑥) / 面雨量预报表。各子接口**独立容错**（单板块失败占位、其余照常、不整图失败）；预报板块起报时次自动回退（500→下一个时次）。

- **新工具**（`haihe-weather-analyzer-mcp/custom_tools/composite_longimg_tool.py`，已注册 `server.py` + `custom_tools/__init__.py`）：`generate_haihe_composite_longimg(beginTime="", endTime="", areaIds=None, interval=24, range="9", type="0")`，返回 `{status, base64, text, render_warning, beginTime, endTime, ...}`。渲染用模板素材四件套（bg/top-bg/title/publish）+ 中文字体探测；缺字体/Pillow 降级 `base64:""` + 各板块文字。base 默认 `http://10.226.107.35:8001`。实时出图**不做 TTL 缓存**。
- **三个长图工具并存的路由边界**（双轨 prompt :408/:1085 等）：任意"长图"话术默认走本工具；只明确要"降水实况文字"→ `generate_rainfall_describe_longimg`；点名"网页版/网址版/hhweb/白底地图"→ `get_haihe_product_image_url`（拼网址截图）。前端 base64 展示统一走 `_render_base64_tool_image`（**一次工具调用=一张图；出现"两张图"= planner 本轮调了两个长图工具**，非渲染 bug）。
- **④⑥ 图例/标题白字问题（根因已双重验证）**：`isClimateImg=True` 已按接口文档以 JSON 布尔传给 ③④⑥，但**服务端只对 ③ 生效**（③ 返回白底黑字）；④⑥ 只切了白底、图例刻度与标题仍是**白字印白底**不可读（用户 143242 实渲染 + 像素测量双重确认）。**"调对 isClimateImg 就出黑字"只对 ③ 成立**，对 ④⑥ 不成立——故对 ④⑥ 出图做 `_to_white_map` 后处理把文字转深。
- **`_to_white_map` 判定原理（当前 c1133f5 版，勿放宽近邻带/勿删 large_white 守卫）**：白底分支下，灰字（achrom 且 90≤max<245，如图例刻度浅灰字 (176,176,192)）恒转深；**纯白字（max≥245）只靠"是否紧贴彩色内容且为细白笔画"判定**——`near` = 彩色内容（饱和色 ∪ 已填黑块）经 `BoxBlur(5)`+阈值得到的**窄**近邻带，再减去 `large_white`（`MinFilter+MaxFilter(15)` 开运算留下的大块白背景），只有"落入窄带且非大块白"的亮字才转深（绿区白字/细白缝）；大型近黑区块（零值填充区）填最浅绿（`_lightest_green`）；黑底图（dark_fraction≥0.35）走黑→白底、浅字→深字分支。**回归警示**：白字与白背景同像素值，只能靠近邻带区分，但**近邻带必须窄（BoxBlur(5)≈2px）且必须保留 large_white 守卫**——2e4b292 曾改用 `MaxFilter(15)` 宽膨胀（≈7px）并删掉守卫，导致每个雨区/色标条被一圈黑边（暗晕）包围，已回退。由 `test_white_bg_no_halo_around_isolated_color` 锁死此行为。
- **前端展示**：`_run_tool_round` 特判 `generate_haihe_composite_longimg`（:2260），`_render_base64_tool_image` 出图；`no_data`/`error` 中文兜底。
- **② 雷达图黑底 + ③ 实况图浅灰字归一化（2026-08-18 晚，全板块对齐示范图"白底黑字彩色数据"）**：② swan3 雷达图无回波区是纯黑大块（要白色），但标题/分区标签也是黑色细笔画，色值无法区分，只能用**连通域面积**——`_radar_black_to_white` 用 `numpy + scipy.ndimage.label`（核心文件已依赖，懒导入+缺失则原样返回不崩溃）把面积 ≥3000 的近黑大连通域填白、细黑字保留、彩色回波不动。**勿用纯阈值**（会把纯黑标题填白）**或 PIL 形态学开/闭运算**（方核 MaxFilter/MinFilter 在回波边缘产块状锯齿，均实测不可行）。③ 降水实况图白底但副标题/图例刻度浅灰，与 ④⑥ 同根因，故也过 `_to_white_map`（副标题暗像素 24→196、图例 270→504 实测转深，分区名/描边/彩色保留）。**现状：②③④⑥ 四张栅格图全部做归一化**（② 黑底转白；③④⑥ 浅字转深/黑块填浅绿）。
- 测试：`haihe-weather-analyzer-mcp/tests/test_composite_longimg_tool.py`（26 条：`_to_white_map` 5 类 + `_radar_black_to_white` 2 类 + 板块归一化接线 1 类 + 板块容错/预报回退/参数透传等）。测试经 importlib 按文件路径加载模块绕开 `custom_tools/__init__.py` 重依赖；本机 `.venv-test` 缺 networkx/rasterio/numpy/scipy，雷达连通域正向用例 `importorskip` 跳过（已用系统 python 实测通过），**只有该文件的测试能全量跑**（其余 `tests/test_*.py` 在 collection 阶段 import `custom_tools/__init__.py` 报 ModuleNotFoundError，属环境限制非回归）。
