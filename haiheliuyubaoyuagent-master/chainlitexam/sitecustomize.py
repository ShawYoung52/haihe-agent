"""Chainlit 启动补丁：修复“上个月面雨量”被面雨量快速路径误判为过去24小时的问题。

Python 启动时会自动尝试导入 sitecustomize。项目通常在 chainlitexam 目录下启动，
因此该文件会在 chain_gzt.py 导入 message_orchestrator 前执行，并替换其中的
_basin_areal_rainfall 快速路径。
"""
...
def _unwrap_tool_result(result: Any) -> Any:
    """兼容 MCP / LangChain 工具返回的 text/list/dict 包装。"""
    data = result
...
def _previous_month_range() -> tuple[str, str, str, str]:
    now = datetime.now()
    ...
    return time_range, label, month, end_prev.strftime("%Y%m%d%H%M%S")
...
def _install_patch() -> None:
    ...
    if not callable(original) or getattr(original, "_last_month_patch_installed", False):
        return
    ...
    async def patched_basin_areal_rainfall_fast_path(user_text: str, tools, messages, callbacks) -> bool:
        ...
        time_range, time_label, month, _end_time = _previous_month_range()
