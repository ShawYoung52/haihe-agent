...
| H02 | 今天晚上滦河有雨吗？ | `query_river_rainfall_forecast` | 裸河名先查全量河道；仅未找到才回退已知河系；今晚18:00—24:00，18时后从当前可用整点起 | 离线通过 / 待内网验收 |
...
- POI 复用既有表格和事实门控；无水位/汛限且无雨时“水位预计平稳”的遗留断言已按控制器裁定做最小修复，不新增查询或改变有数据的风险建议。旧 POI no_data 展示语义仍待独立最终审查判断真实契约，未与新区域风险归一化器合并。
