...
> 执行状态（2026-08-28）：河流/河系实现和离线回归完成，独立最终审查待执行，真实PostGIS/GDAL与内网栅格待验证；最终状态、56题清单和命令见[验收记录](../2026-08-27-priority-acceptance.md)。下方保留原始提案步骤，不以未勾选复选框表示尚未实现。最终业务口径为：九分区九个名称（含裸名称）直接按分区统计；下一级具体河流查全量表，采用geography真实米制5000米缓冲，仅未找到才回退所属九分区。新区域风险no_data为不可用，Chainlit/MCP必须分目录分进程使用系统Python测试。
