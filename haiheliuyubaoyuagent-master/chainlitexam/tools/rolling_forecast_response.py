...
def rolling_forecast_llm_instruction(bundle: dict | None) -> str:
    ...
    return (
        ...
        # 2026-08-31 用户口径：回答要符合气象专业、不要"很水很抽象"。一句话也要用规范
        # 天气术语体现天气过程与时段变化，避免空泛评价；气温可给整数区间，降水/大风点明
        # 量级与时段。仍只依据权威事实、不编造数值或天气系统，不重复表格细节。
        "表述要专业：用规范天气术语写出天气过程与时段（如“多云转阴”“午后到夜间有雷阵雨”"
        "“降水主要集中在夜间”“偏南风3到4级”“最高气温32℃、最低气温24℃”），避免“天气不错”"
        "“气温适宜”“比较稳定”这类空泛评价；在用户询问范围内可给出整数气温与风力量级。"
        "你不得生成任何表格、表头、"
