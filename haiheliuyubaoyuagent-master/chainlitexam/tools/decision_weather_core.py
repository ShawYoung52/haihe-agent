...
DECISION_WEATHER_PREFILTER_SUFFIXES = (
    ...
    "度假区", "古镇",
    # 裸"天津港"（无"港口/港区"后缀）：与 MCP 侧 POI_PLACE_KEYWORDS 同口径，
    # "天津港明日风力多大"必须路由到点位决策天气，不得落天津市区代表点。
    "天津港",
)
...
_DECISION_WEATHER_SUFFIXES = [
    ...
    # 裸"天津港"：suffix 即点位全名（"天津港明日风力多大" head 为空，候选=后缀本身）。
    "天津港",
]
# 本身即是完整点位名的后缀（head 为空时也可直接作为位置名，避免回退 LLM 抽槽）。
_DECISION_WEATHER_SELF_NAMED_SUFFIXES = ("天津港",)
...
def _extract_decision_slots_rule_based(user_text: str) -> dict | None:
    ...
    for suffix in sorted(_DECISION_WEATHER_SUFFIXES, key=len, reverse=True):
        ...
        # "本周末/下周末/这周末"必须整体切分（2026-08-26 领导问题清单缺陷："本周末适合去
        # 泰达航母主题公园"只切"本周"残留"末适合去…"污染 POI 检索）。
        head = t[:idx]
        head = re.split(
            r"[，。？?！!、\s，]|今天|明天|后天|昨天|前天|昨日|周末|未来|现在|上午|下午|晚上|夜里|"
            r"高考期间|中考期间|考试期间|"
            r"下周(?:末|[一二三四五六日天])?|下星期[一二三四五六日天]?|本周(?:末|[一二三四五六日天])?|"
            r"这周(?:末|[一二三四五六日天])?|星期[一二三四五六日天]|周[一二三四五六日天]|"
            r"[一二三四五六七八九十\d]+(小时|天|日|周|月|号|年)",
            head,
        )[-1]
        # 去掉前导出行引导词（"本周末适合去X"切分后残留的"适合去/想去"）。
        # 必须先于尾端"去"剥离：否则"能去/可以去/应该去/建议去"的"去"被尾端正则先吃掉，
        # 残留的"能/可以/应该/建议"不在虚词表、被拼进位置名（2026-08-26 code-review：
        # "明天能去天津港"→"能天津港"，POI 检索张冠李戴）。
        head = re.sub(r"^(?:适合去|适宜去|适合|适宜|想去|建议去|可以去|能去|应该去)", "", head)
        # 去掉结尾的"在/去/到/位于/附近"等
        ...
        # 自命名后缀（如"天津港"）本身即完整点位名，head 为空时也可直接采用。
        if candidate and len(candidate) >= 2 and (head or suffix in _DECISION_WEATHER_SELF_NAMED_SUFFIXES):
            location = candidate
            break
