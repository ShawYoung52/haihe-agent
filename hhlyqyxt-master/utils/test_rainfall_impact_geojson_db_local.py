...
- 同一 objectid 或同一河名同时直接/间接出现时，保留直接河流。
...
def build_db_test_outputs(args: argparse.Namespace) -> dict:
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            direct_river_set = set(direct_rivers)
            ...
            downstream_segments = [
                ...
                and str(seg.get("river_name") or "").strip() not in direct_river_set
            ]
            downstream_map = {
                name: info for name, info in downstream_map.items()
                if str(name).strip() not in direct_river_set
            }
    ...
    for row in downstream_rows:
        ...
        if (objectid and objectid in direct_objectids) or (river_name and river_name in direct_river_set):
            continue
