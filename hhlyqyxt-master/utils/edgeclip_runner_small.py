...
说明：历史文件名保留为 edgeclip_runner_small，但当前口径是：
- 30km 缓冲区只判断直接影响，不截断直接河流；
- 渲染几何默认使用 haihe_river_directed_full_v5；
- 下游 50km 最后一段仍截断。
"""
...
def run_edgeclip(
    *,
    ...
    db_srid: int = 4326,
    river_edge_table: str = "haihe_river_directed_full_v5",
    river_geom_column: str = "geom",
    ...
    impact_df = station_df[
        (station_df["rain_24h"] >= rain_threshold_mm)
    ...
    if stations:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            if not columns:
                raise ValueError(f"未找到河流表：{db_schema}.{river_edge_table}")
            create_station_temp(cur, stations, srid=db_srid)
            direct_rows = query_direct_edges_full(
                cur,
                schema=db_schema,
                table=river_edge_table,
                columns=columns,
                geom_column=river_geom_column,
            ...
            downstream_segments = [
                x for x in downstream_segments
                if str(x.get("objectid") or "") not in direct_objectids
                and str(x.get("river_name") or "") not in direct_rivers
            ]
            downstream_map = {k: v for k, v in downstream_map.items() if str(k) not in direct_rivers}
            downstream_rows = query_downstream_edges_clipped(
                cur,
                schema=db_schema,
                table=river_edge_table,
                columns=columns,
    ...
    for row, kind in [(x, "direct_buffer") for x in direct_rows] + [(x, "downstream_50km") for x in downstream_rows]:
        ...
        if kind == "direct_buffer":
            feature["properties"]["geometry_source"] = "full_table_direct_30km_hit_uncut"
    ...
    summary = {
        ...
        "outputs": {"river_geojson": str(river_path), "summary_json": str(summary_path)},
    }
