...
## 2026-09-03：九分区河系降雨统计口径统一

- 大清河、子牙河、永定河、北三河、漳卫南运河、徒骇马颊河、黑龙港、滦河、海河九个分区名称，无论是否带“流域/河系”后缀，均直接按 `haihe_zone_9` 面范围统计，不再因 full_v6 是否存在同名河道而随机切换为两侧5公里走廊。
- “海河流域/全流域”保持查询全部九分区；“海河/海河干流”按海河单分区处理。泃河、潮白河、蓟运河、滹沱河等下一级具体河流仍优先使用真实河道两侧5公里，走廊未找到时才回退所属九分区。

## 2026-09-03：于桥水库日期与蓟州综合风险回答修复

- POI 多日决策天气的核心结论会按预报事实为每个逐日日期补全月份，例如“9月4日、9月5日、9月6日”；跨月按各自真实月份展示，并避免把“未来3日”等时长误判为日期。原有逐日表格、风险和建议结构保持不变。
- “今天蓟州可能有哪些风险”等区域综合风险仍只调用 `query_region_weather_risks`，该工具内部复用滚动预报并返回 `weather_forecast`；回答先给真实天气及风险类型的核心结论，再展开地质灾害、山洪和中小河流三类详情。天气查询失败时保留风险结果并明确降级，不调用天河、不猜测天气，也不重复查询三类风险。
- 已补日期补全、跨月、时长防误改、天气事实透传、风险单次查询、天气失败降级和默认回答提示词回归测试；生产内网数据仍需部署后复测。
