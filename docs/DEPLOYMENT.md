...
```text
haiheliuyubaoyuagent-master/chainlitexam/chainlit_app.py
```

`chainlit_app.py` 会先读取 `settings.py`，再导入原业务入口 `chain_gzt.py`。后续启动脚本、Dockerfile 和 docker-compose 均已切到该入口。
...
cp .env.example .env
# 修改 .env 后再启动
chainlit run chainlit_app.py --host 0.0.0.0 --port 8003
```
...
3. 模型凭证、数据库密码、管理员密码必须通过 Secret 或配置中心注入。
...
5. 启动 chainlitexam/chainlit_app.py
